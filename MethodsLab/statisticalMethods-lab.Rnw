...
% graphics.off();rm(list=ls());library('knitr');knit('statisticalMethods-lab.Rnw');  for(i in 1:2) system('R CMD pdflatex statisticalMethods-lab.tex'); purl('statisticalMethods-lab.Rnw')
...
<<meanSd_Sim_Data, dependson="simulateRNASeq", echo = TRUE>>=
meanSdPlot(counts(dds), ylim = c(0,100))
@
...
rownames(dists) <-  colData(dds)$sex 
colnames(dists) <-  colData(dds)$condition
...
load(url("http://www-huber.embl.de/users/klaus/BasicR/bottomly_eset.RData"))
bottomly.eset
pData(bottomly.eset)
...
<<normalize Bottomly Data, echo = TRUE>>=
...
<<meanSd Bottomly Data, echo = TRUE>>=
meanSdPlot(exprs(bottomly.eset), ylim = c(0,100))
@
...
<<meanSdlog2 Bottomly Data, echo = TRUE>>=
...
<<meanSdlog2 + pseudocount Data, echo = TRUE>>=
...
<<bottomly heatmap and clustering, fig.height= 5>>=
