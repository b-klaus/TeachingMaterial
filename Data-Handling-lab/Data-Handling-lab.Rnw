...
# get indeces of the genes from the SE object in the annotation object
idx_se_in_dm_genes <- match(names(rowRanges(pasilla_se)), names(dm_genes))

# replace NAs with NULLs
#idx_se_in_dm_genes <- ifelse(is.na(idx_se_in_dm_genes), NULL, idx_se_in_dm_genes)

# test <- map(seq_along(idx_se_in_dm_genes), function(x){
#   if(is.na(x)){
#   return(x)
#   }
#   dm_genes[[x]]
...
dm_genes_pasilla <- dm_genes[]

dm_genes[sample(length(dm_genes), 5)]

rowRanges(pasilla_se) <- dm_genes[match(rowRanges(pasilla_se), names(dm_genes)]
@
%
...
that the purrr package provides a family of functions to do it for you. Each function always returns the same type of output so there are six variations based on what sort of result you want:
