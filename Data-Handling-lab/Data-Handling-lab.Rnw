...
% graphics.off();rm(list=ls());library('knitr');knit('Data-Handling-lab.Rnw');purl('Data-Handling-lab.Rnw');for(i in 1:2) system('R CMD pdflatex Data-Handling-lab.tex')
...
handling techniques. Many of the techniques discussed in this lab are very well
elaborated on in the upcoming book "R for data science" available on this \href{http://r4ds.had.co.nz/iteration.html#the-map-functions}{website}.
...
We use the function \Rfunction{read\textunderscore csv} from the \CRANpkg{readr}
package that provides consistent and fast data import functions that do
not depend on your locale settings and option such as stringsAsFactors.
...
data at once by using an appropriate \Rfunction{apply} command.
...
\section{Handling lists and the intelligent \Rfunction{apply} functions with the \CRANpkg{purrr} package}

Lists are the data structure R uses for hierarchical objects.
Lists extend ordinary vectors to model objects that are like trees.
You can create a hierarchical structure with a
list because unlike vectors, a list can contain other lists.

There are three ways to subset a list:

\begin{itemize}
\item \Rcode{[} extracts a sub-list. The result will always be a list.
\item \Rcode{[[} extracts a single component from a list. It removes
a level of hierarchy from the list
\item \Rcode{\$} is a shorthand for extracting named elements of a list. It works similarly to [[ except that you don’t need to use quotes.
\end{itemize}

which we will illustrate with the
following example
...
Looping over a list and doing something to each element is a very  common operation
in R. Unfortunately, the basic R functions like \Rfunction{apply, sapply, lapply}
do neither offer a consistent interface nor a reliable return type.

The \CRANpkg{purrr} package provides a family of \Rfunction{map\textunderscore *} functions to
to provide a consistent interface to list apply functions. They are called
\Rfunction{map\textunderscore *}
since each steps consists of ``mapping'' a list value to a result. Detailed
information on them can be found
\href{http://r4ds.had.co.nz/iteration.html#the-map-functions}{on this website}.

Each function always returns the same type of output so there variations based
on what sort of result you want. Each function takes a list as input, applies
a function to each piece, and then returns a new vector that’s the same length
as the input. The type of the vector is determined by the specific map function.
Usually you want to use the most specific available, using the default
\Rfunction{map()} only as a fallback when there is no specialized
equivalent available. Since a \Robject{data.frame} is just a special kind of list,
We can use e.g. to compute the means of all the variables using \Rfunction{map\textunderscore dbl}
that is guaranteed to return a double value:
...
The mapping functions have additional convenience feature like the automatic
creation of simple functions as well as functions like \Rfunction{flatten} to
remove a level of the hierarchy of the list (similar to \Rfunction{unlist}).

If you specify and index instead of a function, it will extract the respective
element, in our example the fifth line of the data set.
...
This convenience function can of course become very handy with nested
and named lists.
...
\subsection{Bioconductor GRanges and summarized experiments}

The Bioconductor calls \Biocpkg{SummarizedExperiment} can be see as
an ExpressionSet variant for sequencing experiments. The feature data
(now called rowRanges) contains \Biocpkg{GenomicRanges} that represent
genomic information on features assayed. In the simplest case, these are
the genomic coordinates of the features. The \Biocpkg{GenomicRanges} offers
a very flexible and nice framework for working with genomic data. A summary
of its capabilities is given in the \href{http://doi.org/10.1371/journal.pcbi.1003118
}{article on it}, as well  as in \href{http://bioconductor.org/packages/3.2/bioc/vignettes/GenomicRanges/inst/doc/GRanges_and_GRangesList_slides.pdf}{this presentation}.

Here we look at simple use case: We start from a drosophila
RNA--Seq data set, \Biocexptpkg{pasilla},
which contains only the Flybase gene identifiers and then add the annotation
of the corresponding transcripts.

We obtain this annotation from the
\Biocannopkg{TxDb.Dmelanogaster.UCSC.dm3.ensGene}. Since the creation of
the pasilla data set dates back to 2012 or so, some gene ids are no longer
present in the current annotation and we have to tackle them so that the matching
of the two id sets works.
...
command and then use \Rfunction{map} with a custom function
to extract the info!


\item Add  columns  \Rcode{gal\textunderscore glu} and \Rcode{green\textunderscore red} with the function \Rfunction{mutate}
...
\item Group the data by the columns \Rcode{gal\textunderscore glu} and
\Rcode{green\textunderscore red} and
