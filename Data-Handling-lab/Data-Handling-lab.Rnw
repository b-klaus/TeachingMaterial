...
#select("TxDb.Dmelanogaster.UCSC.dm3.ensGene", keys = ids_pasilla,
#       keytype = "GENEID",
#       )
...
#names(rowRanges(pasilla_se))[100] = "XXX"
...
We use the function \Rfunction{read\textunderscore csv} from the \CRANpkg{readr} package that provides
consistent and fast data import functions.
...
The \CRANpkg{purrr} package provides a family of \Rfuntion{map_*} functions to
to provide a consistent interface to list apply functions. They are called \Rfuntion{map_*}
...
We can use e.g. to compute the means of all the variables using \Rfuction{map_dbl}
